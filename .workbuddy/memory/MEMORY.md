...
- **发布**：`.dump/_deploy_index_now.py` 部署 index.html+sw.js+README.md 三文件（Git Database API，`gh` 前必须 `env -u` 清全部代理）；新增数据文件记得加部署 FILES 清单。**同步差异检查**：`.dump/_sync_docs.py`（FILES 全量清单）；确认「哪些没传/哪些过时」用 blob sha 全量比对法（skill「确认 GitHub 与本地的差异」节）。**同步差异检查**：`.dump/_sync_docs.py`（FILES 全量清单）；确认「哪些没传/哪些过时」用 blob sha 全量比对法（skill「确认 GitHub 与本地的差异」节）。
