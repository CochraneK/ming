...
- **发布**：`.dump/_deploy_index_now.py`（index.html+sw.js+README.md，Git Database API，3 次重试，内部清代理，`MING_DEPLOY_MESSAGE=` 可覆盖）；`.dump/_sync_docs.py` 全量清单（`MING_SYNC_MESSAGE=` 可覆盖）；**新增文件必须补 FILES 清单**（本轮已补 `src/core/faction_profile.py`、`src/core/graph_layout.py`、`report/Ming_重构验收清单.md`），否则 CI 报缺文件。差异检查 `.dump/_diff_remote.py`（期望「过时 0」）。
  - **`_sync_docs.py` 是超集**：其 FILES 已含 `index.html`/`sw.js`/`README.md`，且走 `POST /git/blobs`（无 Contents API 的 1MB 限制）。**只跑它即可完成发布**；再跑 `_deploy_index_now.py` 只会多一个内容相同的空提交。（`_deploy_index_now.py` 保留作"只改前端"时的轻量通道。）
  - **核验线上内容必须绕 CDN 缓存**：Pages 边缘 `max-age=600`，刚部署后直接拉 URL 可能返回**上一版**（实测 `sw.js` 拿到 1861 B 旧字节）。加查询串（`…/sw.js?v=1`）即可穿透；或直接读 `raw.githubusercontent.com/<owner>/<repo>/main/<path>`。**注意浏览器 SW 更新检查本身绕过 HTTP 缓存**，所以 SW 能拿到新版，不要据此误判部署失败。
