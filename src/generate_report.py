...
const pageSize=36;
const state={dynastyEra:'',mapMode:'pilgrim',showMarkers:true,view:'overview',locMode:'index',locPage:1,locQuery:'',locRegion:'全部区域',chapterPage:1,distributionPage:1,distributionPart:'全部七部',distributionSort:'order',visualPart:'全部',visualPerson:'',netMode:'ego',charPage:1,charQuery:'',charPart:'',charFaction:'全部势力',charMinor:false,eventPage:1,eventQuery:'',eventType:'全部类型',eventCategory:'全部类别',relationPage:1,relationQuery:'',relationCategory:'全部类别',timelinePage:1,timelineCategory:'全部类别',printMode:null,rendered:{}};
...
function renderCharacters(){
 ...
 $('#charPart').addEventListener('change',e=>{state.charPart=e.target.value;state.charPage=1;renderCharacters()});$('#charFaction').addEventListener('change',e=>{state.charFaction=e.target.value;state.charPage=1;renderCharacters()});$('#charQuery').addEventListener('input',e=>{state.charQuery=e.target.value;state.charPage=1;renderCharacters()});$('#charReset').addEventListener('click',()=>{state.charPart='';state.charFaction='全部势力';state.charQuery='';state.charMinor=false;state.charPage=1;renderCharacters()});$('#charMinor').addEventListener('change',e=>{state.charMinor=e.target.checked;state.charPage=1;renderCharacters()});
