...
.dynasty-band{display:flex;gap:2px;height:52px;align-items:stretch;margin-bottom:6px}.dynasty-labels{position:relative;height:36px;margin-bottom:10px}.dynasty-label{position:absolute;transform:translateX(-50%);white-space:nowrap;font-size:11px;line-height:15px;color:#5f5145;cursor:pointer;padding:1px 5px;border-radius:4px;transition:background .15s,color .15s}.dynasty-label.row-0{top:0}.dynasty-label.row-1{top:18px}.dynasty-label:hover{background:#f0e7d8;color:var(--accent)}.dynasty-label.active{background:var(--accent);color:#fff5ec}
...
<main class="shell">
  ...
  <section id="map" class="view"><div class="section-head"><div><h2>地图</h2><p>圣地巡礼地图聚焦书中已定位地点；郑和下西洋航线为独立子视图，仅绘制航线与停靠点。</p></div></div><div class="subnav" id="mapSubnav"><button class="active" data-map-mode="pilgrim">圣地巡礼地图</button><button data-map-mode="voyage">郑和下西洋航线</button></div><div class="panel map-wrap" id="pilgrimPanel"><div class="toolbar"><label style="display:inline-flex;gap:6px;align-items:center;color:var(--muted)"><input type="checkbox" id="markerToggle" checked>显示地点标记</label><span class="muted grow" id="mapNote"></span></div><div class="map-layout"><div class="map-main"><div id="mapBox" class="map-box"></div></div><aside id="locDock" class="loc-dock" hidden></aside></div></div><div class="panel map-wrap" id="voyagePanel" style="display:none"><div class="toolbar"><span class="muted grow" id="voyageNote"></span></div><div class="map-layout"><div class="map-main"><div id="voyageBox" class="map-box"></div></div><aside id="voyageDock" class="loc-dock" hidden></aside></div></div></section>
...
const pageSize=36;
const state={dynastyEra:'',mapMode:'pilgrim',showMarkers:true,cleanExpanded:false,view:'overview',locMode:'index',locPage:1,locQuery:'',locRegion:'全部区域',chapterPage:1,distributionPage:1,distributionPart:'全部七部',distributionSort:'order',visualPart:'全部',visualPerson:'',netMode:'ego',charPage:1,charQuery:'',charPart:'',charFaction:'全部势力',charMinor:false,eventPage:1,eventQuery:'',eventType:'全部类型',eventCategory:'全部类别',relationPage:1,relationQuery:'',relationCategory:'全部类别',timelinePage:1,timelineCategory:'全部类别',printMode:null,rendered:{}};
...
const markerToggleEl=$('#markerToggle');if(markerToggleEl){markerToggleEl.addEventListener('change',e=>{state.showMarkers=e.target.checked;renderMap()})}
...
function renderCharacters(){
 ...
 $('#charPart').addEventListener('change',e=>{state.charPart=e.target.value;state.charPage=1;renderCharacters()});$('#charFaction').addEventListener('change',e=>{state.charFaction=e.target.value;state.charPage=1;renderCharacters()});bindSearch('#charQuery','charQuery',()=>{state.charPage=1;renderCharacters()});$('#charReset').addEventListener('click',()=>{state.charPart='';state.charFaction='全部势力';state.charQuery='';state.charMinor=false;state.charPage=1;renderCharacters()});$('#charMinor').addEventListener('change',e=>{state.charMinor=e.target.checked;state.charPage=1;renderCharacters()});
...
function characterCard(x){
 ...
 return `<article class="character-card"><div class="inner">
  <div class="face front">
   <div class="pc-head"><span class="pc-faction">${esc(x.faction||'势力待补')}</span><span class="pc-seal" aria-hidden="true">明</span></div>
...
// 年号标签移到条下方，按可用宽度自动错行：条宽即在位年数，短命年号只有 1~2% 宽，标签挤在条里必然糊成一团
...
const eraLabels=_lrows[0].join('')+_lrows[1].join('');
...
$('#dynasty').innerHTML=`<div class="section-head"><div><h2>帝王谱系</h2><p>十六位天子、十七段年号；${DATA.timeline.length} 件可纪年大事按在位期自动归位，条带宽度即在位时长。</p></div><span class="status">帝系为人工整理 · 事件自动对位</span></div><div class="panel" style="margin-bottom:14px"><div class="section-head"><div><h2 style="font-size:16px">一、在位条带与逐年大事热度</h2><p>上条为年号区间（1368-1644），下条为逐年事件数，颜色随年号切换；悬停看具体年份。</p></div></div><div class="dynasty-band">${segs}</div><div class="dynasty-labels">${eraLabels}</div><div class="dynasty-hist">${bars}</div><div class="dynasty-scale"><span>1368 · 洪武开国</span><span>1644 · 崇祯殉国</span></div></div><div class="panel"><div class="section-head"><div><h2 style="font-size:16px">二、十六帝小传</h2><p>点击任一帝王卡片或上方年号条，展开本朝大事与活跃人物。</p></div></div><div class="dynasty-cards">${cards}</div></div>${detail}`;
