...
def build_scope(scope: str):
    ...
    for stop in voyages_meta.get("stops", []):
        ...
        if loc and loc.get("lng") is not None and loc.get("lat") is not None:
            ...
            voyage_points.append({"name": stop, "lng": lng, "lat": lat, "desc": desc})
...
function renderMap(){
  ...
  $('#mapNote').textContent=`已定位 ${locations.length}/${DATA.locations.length} 个地点。${window.L?'地图瓦片来自 OpenStreetMap，需联网加载。':'当前为离线点位图，地图瓦片未加载。'}`;
  const box=$('#mapBox');
...
function showVoyageStop(p,i){const dock=$('#voyageDock');dock.innerHTML=`<div class="dock-head"><h3>${i+1}. ${esc(p.name)}</h3><button class="dock-close" id="voyageDockClose">关闭</button></div><div class="detail-grid"><div class="detail-block detail-wide"><strong>停靠序号</strong><p>第 ${i+1} 站 / 共 ${DATA.voyages.points.length} 站</p></div><div class="detail-block detail-wide"><strong>书中描述</strong><p>${esc(p.desc||'（书中仅提及，未详述）')}</p></div><div class="detail-block"><strong>坐标</strong><p>${Number(p.lat).toFixed(2)}, ${Number(p.lng).toFixed(2)}</p></div><div class="detail-block"><strong>核验</strong><p class="muted">${DATA.voyages.illustrative?'坐标为示意，需史料核验':'坐标已核验'}</p></div></div>`;dock.hidden=false;$('#voyageDockClose').addEventListener('click',()=>{dock.hidden=true})}
...
function characterCard(x){
 ...
 const evLine=x.events.length?esc(x.events.slice(0,4).join('、')):`<span class="muted">${esc((x.contextEvents||[]).slice(0,4).join('、')||'无')}</span><span class="src-note">（同章提及）</span>`;
 const relLine=esc(x.relations.slice(0,4).map(r=>`${r.to}（${r.rel}）`).join('、')||'无');
