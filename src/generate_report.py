...
.character-card{height:auto!important;page-break-inside:avoid;perspective:none!important;box-shadow:none!important;overflow:visible!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
...
function renderPrint(kind,list){const pages=[];for(let i=0;i<list.length;i+=9){const cards=list.slice(i,i+9).map(x=>{const F=cleanCardFields(x);return kind==='front'?`<div class="print-card front"><div class="name">${esc(x.name)}</div><div class="text">${esc(x.summary)}</div></div>`:`<div class="print-card back"><div class="name">${esc(x.name)}</div><div>身份：${esc(F.role)}</div><div>势力：${esc(F.faction)}</div><div>生卒：${esc(x.life)}</div><div>籍贯：${esc(F.birth)}</div>${F.extra&&F.extra!==F.birth?`<div>出身：${esc(F.extra)}</div>`:''}<div>事件：${esc(x.events.slice(0,5).join('、')||'无')}</div><div>关系：${esc(x.relations.slice(0,5).map(r=>`${r.to}（${r.rel}）`).join('、')||'无')}</div></div>`}).join('');pages.push(`<div class="print-page"><div class="print-grid">${cards}</div></div>`)}$('#printArea').innerHTML=pages.join('')}`
