...
function renderCharacters(){
 const factions=[...new Set(DATA.characters.map(x=>x.faction).filter(Boolean))].sort();const list=DATA.characters.filter(x=>(!state.charPart||x.chapters.some(k=>k.startsWith(state.charPart+'-')))&&(!state.charFaction||state.charFaction==='全部势力'||x.faction===state.charFaction)&&(!state.charQuery||x.name.toLowerCase().includes(state.charQuery.toLowerCase()))&&(state.charMinor||state.charQuery.trim()||x.chapters.length>1));const page=slicePage(list,state.charPage);$('#characters').innerHTML=`<div class="section-head"><div><h2>人物索引</h2><p>${metrics.characters} 个角色实体；默认按章节覆盖度排序，关系完整保留在详情中。</p></div></div><div class="panel"><div class="toolbar"><label>部次</label><select id="charPart"><option value="">全部七部</option>${Object.entries(PARTS).map(([k,v])=>`<option value="${k}" ${state.charPart===k?'selected':''}>${esc(v)}</option>`).join('')}</select><label>势力</label><select id="charFaction"><option>全部势力</option>${factions.map(x=>`<option ${state.charFaction===x?'selected':''}>${esc(x)}</option>`).join('')}</select><input class="search grow" id="charQuery" value="${esc(state.charQuery)}" placeholder="按姓名搜索"><button class="action" id="charReset">重置</button></div><div class="character-toolbar"><button class="mode active" data-char-mode="screen">屏幕卡</button><button class="mode" data-char-mode="front">打印正面</button><button class="mode" data-char-mode="back">打印背面</button><label style="display:inline-flex;gap:5px;align-items:center;color:var(--muted)">卡面</label><select id="cardTheme"><option value="thm-ink" ${state.cardTheme==='thm-ink'?'selected':''}>墨玉</option><option value="thm-paper" ${state.cardTheme==='thm-paper'?'selected':''}>宣纸</option><option value="thm-cinnabar" ${state.cardTheme==='thm-cinnabar'?'selected':''}>朱砂</option></select><label><input type="checkbox" id="charMinor" ${state.charMinor?'checked':''}> 显示仅出场一章的人物</label><span class="muted" id="charCount">显示 ${page.length} / ${list.length} · 次要(仅1章) ${DATA.characters.filter(x=>x.chapters.length<=1).length} 人${state.charMinor||state.charQuery.trim()?'':'（已折叠）'}</span></div><div id="charScreen" class="character-grid">${page.map(characterCard).join('')||'<div class="empty">没有匹配人物</div>'}</div><div id="printGuide" class="print-guide" style="display:none">当前筛选结果共 ${list.length} 张；打印页只在选择打印模式时生成。</div><div id="printArea" class="print-area"></div>${pager(state.charPage,list.length)}</div>`;
