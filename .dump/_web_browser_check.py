# -*- coding: utf-8 -*-
"""V10 在线人物/地点摘要、详情 shard 与空间模式按需的真实 Chrome 冒烟。"""
...
def _command_probe(click=False):
    ...
    p=TARGET.parent/('.v10-command-click.html' if click else '.v10-command.html');p.write_text(doc.replace('</body>',probe+'</body>',1),encoding='utf-8');return p
...
def check_search_only():
    ...
    print('ok   V10 Ctrl+K 仅 search-index')
...
def check_character_index():
    ...
    assert_no_chunk(dom,*CHAR_SHARDS,*LOC_SHARDS,'locations','place-chapters','voyages','events','relations','time','graphs','insight','meta')
    print('ok   V10 人物索引仅 cards')
...
def check_person_detail():
    ...
    assert_no_chunk(dom,'locations',*LOC_SHARDS,'place-chapters','voyages','relations','time','graphs','meta')
    print('ok   V10 于谦仅人物 shard-13')
...
def check_location_index():
    ...
    assert_no_chunk(dom,*LOC_SHARDS,'place-chapters','voyages','events','insight','relations','time','graphs','meta')
    print('ok   V10 地点索引仅 locations 摘要')
...
def check_place_detail():
    ...
    assert_no_chunk(dom,'place-chapters','voyages','relations','time','graphs','meta')
    print('ok   V10 宁远详情仅 location shard-00 + events/insight')
...
def check_map_core_only():
    ...
    assert_no_chunk(dom,*LOC_SHARDS,'place-chapters','voyages','events','insight','relations','time','graphs','meta')
    print('ok   V10 默认地图仅 locations 摘要')


def _click_probe(selector,delay=900):
    doc=TARGET.read_text(encoding='utf-8')
    probe="<script>setTimeout(function(){var b=document.querySelector(%r);if(b)b.click();},%d);</script>\n"%(selector,delay)
    p=TARGET.parent/'.v10-click-probe.html';p.write_text(doc.replace('</body>',probe+'</body>',1),encoding='utf-8');return p
...
def check_location_chapter_mode():
    ...
    assert_no_chunk(dom,*LOC_SHARDS,'voyages','events','insight')
    print('ok   V10 地点按章节 → 仅增量 place-chapters')


def check_chronicle():
    dom=chrome_dump(TARGET,'#view=chronicle',9500)
    assert_has(dom,r'data-ming-chunks="characters,time"',r'data-ming-data-chunk="characters"',r'data-ming-data-chunk="time"',r'年谱 · 人物生平对照')
    assert_no_chunk(dom,*CHAR_SHARDS,'locations',*LOC_SHARDS,'events','graphs','insight','meta')
    print('ok   V10 年谱保持 time+characters')
...
def check_graph():
    ...
    assert_no_chunk(dom,'characters',*CHAR_SHARDS,'locations',*LOC_SHARDS,'events','time','insight','meta')
    print('ok   V10 图谱仅 graphs')
...
def check_timeline():
    ...
    assert_no_chunk(dom,'characters',*CHAR_SHARDS,'locations',*LOC_SHARDS,'graphs','insight','meta')
    print('ok   V10 时间轴仅 events+time')
